...
+ (NSString *)createSimulateIDFA{
    NSString *sysBootTime = systemBootTime();  //获取系统启动时间
    NSString *countryC= countryCode();         //获取国家代码
    NSString *languge = language();            //获取本地语言
    NSString *deviceN = deviceName();          //获取设备名称
    
    NSString *sysVer = systemVersion();        //获取系统版本
    NSString *systemHardware = systemHardwareInfo();  //获取机型、运营商名称、内存
    NSString *systemFT = systemFileTime();            //获取 coreServices 文件创建更新时间
    NSString *diskS = disk();                         //获取硬盘使用空间
